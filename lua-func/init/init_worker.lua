...
local g_event_handle_model = require("event-func.event_handle_model")
local heart_time = 60
...
--event请求的异步处理
local ok,err = ngx.timer.every(0.5, g_event_handle_model.event_handle_thread)
if not ok then
    ngx.log(ngx.ERR,"event handle thread, err:", err)
    return
end
